<template>
  <div class="space-y-6">
    ...
    <section class="rounded-2xl border border-white/10 bg-white/5 p-6">
      <h2 class="text-xl font-semibold mb-4">{{ t('subscriptions.userInfo') }}</h2>
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="rounded-xl border border-white/10 bg-midnight p-4">
          <p class="text-sm text-slate-400">{{ t('subscriptions.plan') }}</p>
          <p class="text-lg font-semibold">{{ user?.subscriptionType || 'N/A' }}</p>
        </div>
        <div class="rounded-xl border border-white/10 bg-midnight p-4">
          <p class="text-sm text-slate-400">{{ t('subscriptions.appsUsed') }}</p>
          <p class="text-lg font-semibold">{{ user?.currentAppCount || 0 }} / {{ user?.maxApps || 0 }}</p>
        </div>
        <div class="rounded-xl border border-white/10 bg-midnight p-4">
          <p class="text-sm text-slate-400">{{ t('subscriptions.translationsUsed') }}</p>
          <p class="text-lg font-semibold">{{ user?.currentUsage || 0 }} / {{ user?.maxTranslations || 0 }}</p>
        </div>
        <div class="rounded-xl border border-white/10 bg-midnight p-4">
          <p class="text-sm text-slate-400">{{ t('subscriptions.status') }}</p>
          <p class="text-lg font-semibold">{{ user?.isSubscribed ? t('subscriptions.active') : t('subscriptions.inactive') }}</p>
        </div>
      </div>
    </section>
    ...
    <!-- Subscription Plans -->
    <section class="rounded-2xl border border-white/10 bg-white/5 p-6">
      <h2 class="text-xl font-semibold mb-4">{{ t('subscriptions.plans') }}</h2>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <!-- Free Plan -->
        <div class="rounded-2xl border border-white/10 bg-midnight p-6">
          <h3 class="text-lg font-semibold mb-2">{{ t('subscriptions.free') }}</h3>
          <p class="text-3xl font-bold mb-4">Free</p>
          
          <ul class="space-y-2 mb-6">
            <li class="flex items-center">
              <span class="text-emerald-400 mr-2">✓</span>
              <span>{{ t('subscriptions.features.upToApps', { count: 1 }) }}</span>
            </li>
            <li class="flex items-center">
              <span class="text-emerald-400 mr-2">✓</span>
              <span>{{ t('subscriptions.features.upToTranslations', { count: 1000 }) }}</span>
            </li>
            <li class="flex items-center">
              <span class="text-emerald-400 mr-2">✓</span>
              <span>{{ t('subscriptions.features.basicSupport') }}</span>
            </li>
          </ul>
        ...
        <div class="rounded-2xl border border-white/10 bg-midnight p-6 relative">
          ...
          <h3 class="text-lg font-semibold mb-2">{{ t('subscriptions.basic') }}</h3>
          ...
          <ul class="space-y-2 mb-6">
            <li class="flex items-center">
              <span class="text-emerald-400 mr-2">✓</span>
              <span>{{ t('subscriptions.features.upToApps', { count: 5 }) }}</span>
            </li>
            <li class="flex items-center">
              <span class="text-emerald-400 mr-2">✓</span>
              <span>{{ t('subscriptions.features.upToTranslations', { count: 10000 }) }}</span>
            </li>
            <li class="flex items-center">
              <span class="text-emerald-400 mr-2">✓</span>
              <span>{{ t('subscriptions.features.prioritySupport') }}</span>
            </li>
            <li class="flex items-center">
              <span class="text-emerald-400 mr-2">✓</span>
              <span>{{ t('subscriptions.features.teamAccess') }}</span>
            </li>
          </ul>
          ...
          >
            <span v-if="user?.subscriptionType === 'basic'">{{ t('subscriptions.current') }}</span>
            <span v-else>{{ t('subscriptions.upgrade') }}</span>
          </button>
        </div>
        ...
        <div class="rounded-2xl border border-white/10 bg-midnight p-6 relative">
          ...
          <h3 class="text-lg font-semibold mb-2">{{ t('subscriptions.premium') }}</h3>
          <p class="text-3xl font-bold mb-4">
          ...
          <ul class="space-y-2 mb-6">
            <li class="flex items-center">
              <span class="text-emerald-400 mr-2">✓</span>
              <span>{{ t('subscriptions.features.upToApps', { count: 20 }) }}</span>
            </li>
            <li class="flex items-center">
              <span class="text-emerald-400 mr-2">✓</span>
              <span>{{ t('subscriptions.features.upToTranslations', { count: 50000 }) }}</span>
            </li>
            <li class="flex items-center">
              <span class="text-emerald-400 mr-2">✓</span>
              <span>{{ t('subscriptions.features.prioritySupport') }}</span>
            ...
            <li class="flex items-center">
              <span class="text-emerald-400 mr-2">✓</span>
              <span>{{ t('subscriptions.features.advancedAnalytics') }}</span>
            </li>
          </ul>
          ...
          >
            <span v-if="user?.subscriptionType === 'premium'">{{ t('subscriptions.current') }}</span>
            <span v-else>{{ t('subscriptions.upgrade') }}</span>
          </button>
        </div>
      </div>
    ...
    <!-- Usage Information -->
    <section class="rounded-2xl border border-white/10 bg-white/5 p-6">
      <h2 class="text-xl font-semibold mb-4">{{ t('subscriptions.usage') }}</h2>
      <div class="space-y-4">
        <div>
          <div class="flex justify-between mb-1">
            <span class="text-sm">{{ t('subscriptions.appsUsed') }}</span>
        ...
        <div>
          <div class="flex justify-between mb-1">
            <span class="text-sm">{{ t('subscriptions.translationsUsed') }}</span>
