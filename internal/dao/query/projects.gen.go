...
func newProject(db *gorm.DB, opts ...gen.DOOption) project {
	...
	_project.projectDo.UseModel(&database.Project{})
	...
	_project.FileContent = field.NewString(tableName, "file_content")
	_project.FileName = field.NewString(tableName, "file_name")
	_project.SourceLanguage = field.NewString(tableName, "source_language")
	_project.ContentStructure = field.NewField(tableName, "content_structure")
	_project.Settings = field.NewField(tableName, "settings")
...
func (p *project) updateTableName(table string) *project {
	...
	p.FileContent = field.NewString(table, "file_content")
	p.FileName = field.NewString(table, "file_name")
	p.SourceLanguage = field.NewString(table, "source_language")
	p.ContentStructure = field.NewField(table, "content_structure")
...
func (p *project) fillFieldMap() {
	...
	p.fieldMap["source_language"] = p.SourceLanguage
	p.fieldMap["content_structure"] = p.ContentStructure
...
func (a projectBelongsToUser) Model(m *database.Project) *projectBelongsToUserTx {
	return &projectBelongsToUserTx{a.db.Model(m).Association(a.Name())}
}

func (a projectBelongsToUser) Unscoped() *projectBelongsToUser {
	a.db = a.db.Unscoped()
	return &a
}
...
func (a projectBelongsToUserTx) Find() (result *database.User, err error) {
	return result, a.tx.Find(&result)
}

func (a projectBelongsToUserTx) Append(values ...*database.User) (err error) {
...
func (a projectBelongsToUserTx) Replace(values ...*database.User) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a projectBelongsToUserTx) Delete(values ...*database.User) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a projectBelongsToUserTx) Unscoped() *projectBelongsToUserTx {
	a.tx = a.tx.Unscoped()
	return &a
}
...
type IProjectDo interface {
	...
	Unscoped() IProjectDo
	Create(values ...*database.Project) error
	CreateInBatches(values []*database.Project, batchSize int) error
	Save(values ...*database.Project) error
	First() (*database.Project, error)
	Take() (*database.Project, error)
	Last() (*database.Project, error)
	Find() ([]*database.Project, error)
	FindInBatch(batchSize int, fc func(tx gen.Dao, batch int) error) (results []*database.Project, err error)
	FindInBatches(result *[]*database.Project, batchSize int, fc func(tx gen.Dao, batch int) error) error
	Pluck(column field.Expr, dest interface{}) error
	Delete(...*database.Project) (info gen.ResultInfo, err error)
	...
	FirstOrInit() (*database.Project, error)
	FirstOrCreate() (*database.Project, error)
	FindByPage(offset int, limit int) (result []*database.Project, count int64, err error)
...
func (p projectDo) Create(values ...*database.Project) error {
	if len(values) == 0 {
		return nil
	}
...
func (p projectDo) CreateInBatches(values []*database.Project, batchSize int) error {
	return p.DO.CreateInBatches(values, batchSize)
}

// Save : !!! underlying implementation is different with GORM
// The method is equivalent to executing the statement: db.Clauses(clause.OnConflict{UpdateAll: true}).Create(values)
func (p projectDo) Save(values ...*database.Project) error {
	if len(values) == 0 {
		return nil
	}
...
func (p projectDo) First() (*database.Project, error) {
	if result, err := p.DO.First(); err != nil {
		return nil, err
	} else {
		return result.(*database.Project), nil
	}
}

func (p projectDo) Take() (*database.Project, error) {
...
func (p projectDo) Last() (*database.Project, error) {
...
func (p projectDo) Find() ([]*database.Project, error) {
	result, err := p.DO.Find()
	return result.([]*database.Project), err
}

func (p projectDo) FindInBatch(batchSize int, fc func(tx gen.Dao, batch int) error) (results []*database.Project, err error) {
	buf := make([]*database.Project, 0, batchSize)
...
func (p projectDo) FindInBatches(result *[]*database.Project, batchSize int, fc func(tx gen.Dao, batch int) error) error {
...
func (p projectDo) FirstOrInit() (*database.Project, error) {
...
func (p projectDo) FirstOrCreate() (*database.Project, error) {
...
func (p projectDo) FindByPage(offset int, limit int) (result []*database.Project, count int64, err error) {
...
func (p projectDo) Delete(models ...*database.Project) (result gen.ResultInfo, err error) {
